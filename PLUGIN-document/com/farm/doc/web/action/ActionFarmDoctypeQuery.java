...
public class ActionFarmDoctypeQuery extends WebSupport {
	...
	public String queryall() {
		try {
			result = aloneIMP.createSimpleTypeQuery(query).search()
					.runDictionary(
							FarmManager.instance().findDicTitleForIndex(
									"FARM_DOCTYE_TYPEMOD"), "TYPEMOD")
					.runDictionary(
							FarmManager.instance().findDicTitleForIndex(
									"FARM_DOCTYE_DOCMOD"), "CONTENTMOD")
		...
		try {
			...
			case (1): {// 新增
				if (ids != null && ids.trim().length() >= 0) {
					fatherEntity = aloneIMP.getType(ids);
				} else {
					fatherEntity = new FarmDoctype();
			...
			case (0): {// 展示
				...
				if (entity.getParentid().equals("NONE")) {
					...
					fatherEntity = aloneIMP.getType(entity.getParentid());
				}
				return SUCCESS;
			}
			case (2): {// 修改
	...
	private static final Logger log = Logger
			.getLogger(ActionFarmDoctypeQuery.class);

		...
		function valueInit() {
			...
			$('select', form).each(function(index, value) {
				$('option', value).each(function(index, val) {
					if (($(val).attr('value') == $(value).attr('val'))&&$(value).attr('val')!="") {
			...
			if (_pageType == _viewType) {
				...
				//日期 
				$(".easyui-datebox",form).each(function(index, value) {
					$(value).after('<div style="white-space: normal;">' + $(value).attr('value').replace("00:00:00.0","") + '</div>');
					$(value).remove();
				});
				//tree
				$('.easyui-combotree', form).each(function(index, value) {	
					var valueStr = $(value).attr('val')
					if (!valueStr) {
						valueStr = '';
					}
					$(value).after('<span>' + valueStr + '</span>');
					$(".easyui-combotree").next().next().remove();
				});
				$('select', form).each(function(index, value) {
				...
				$(':text', form).each(
						function(index, value) {
							$(value).after('<div style="white-space: normal;">' + $(value).attr('value') + '</div>');
							$(value).remove();
						});

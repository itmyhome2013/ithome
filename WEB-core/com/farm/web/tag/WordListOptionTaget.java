...
public class WordListOptionTaget extends TagSupport {
	...
	public int doEndTag() throws JspException {
		...
		list = FarmManager.instance().findWordList();
		...
		for (Iterator iterator = list.iterator(); iterator.hasNext();) {
			...
			try {
				if (isTextValue.trim().equals("1")) {
					...
					jspw.println("<option value='" + type.get("NAME") + "' title='"+DocumentConfig.getString("config.doc.dir").replace("/", "\\") + type.get("DIR")+"'>"
